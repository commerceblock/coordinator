...
pub(crate) mod tests {
    ...
    /// Generate dummy hash
    pub(crate) fn gen_dummy_hash(i: u8) -> sha256d::Hash {
        sha256d::Hash::from_slice(&[i as u8; 32]).unwrap()
    }

    /// Generate dummy challenge state
    pub(crate) fn gen_challenge_state(request_hash: &sha256d::Hash) -> ChallengeState {
    ...
    pub(crate) fn gen_challenge_state_with_challenge(request_hash: &sha256d::Hash, challenge_hash: &sha256d::Hash) -> ChallengeState {
        let request = ServiceRequest {
